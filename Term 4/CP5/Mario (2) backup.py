...
def price_to_get(cur_position, target_position, k, prices):
    ...
    # Пока не будут определены стоимости добраться во все элементы до точки назначения
    z = 0
    while z != len(price_to_come):
        ...
        for y in range(cur_position + k + 1, target_position + 1):
            ...
            possible_prices = []
            for j in range(0, k): # Рассматриваю k предыдущих цен
                possible_prices.append(price_to_come[j] + prices[y]) # TODO: Проблема с индексами prices
            price_to_come[z] = max(possible_prices)
            z += 1
...
def path_to_target(cur_position, target_position, price_to_come):
    ...
    to_be_visited.pop() # Уже нахожусь в там.
...
while cur_position != (n - 1): # Пока не на последнем столбике
    ...
    price_to_come.insert(0, 0) # Добавление в начало списка 0. Т.к. оставаться в тек. позиции ничего не стоит.
